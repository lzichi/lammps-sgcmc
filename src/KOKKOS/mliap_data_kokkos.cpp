...
{
  if (!dev)
    throw std::runtime_error("mliap_kokkos_set_custom_output: dev is null");

  // dev->pairmliap is a PairMLIAPKokkos<LMPDeviceType>*
  auto *pair = dev->pairmliap;
  if (!pair)
    throw std::runtime_error("mliap_kokkos_set_custom_output: pairmliap is null");

  MLIAPData *base = pair->get_data();
  if (!base)
    throw std::runtime_error("mliap_kokkos_set_custom_output: get_data() returned null");
